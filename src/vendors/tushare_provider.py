...
from ..core.models import Bar, Tick, TradeCalendar, Adjustment, Fundamental, Exchange, AdjustmentType
...
class TushareProvider(DataProvider):
    ...
    def get_history(
        ...
        cols = ['dt', 'open', 'high', 'low', 'close', 'volume', 'amount']
    ...
    def get_calendar(
        ...
        calendars = []
        for _, row in df.iterrows():
            calendars.append(TradeCalendar(
    ...
    def get_adjustments(
        ...
        # Tushare adj_factor
        df = self.pro.adj_factor(ts_code=symbol, start_date=start.strftime('%Y%m%d'), end_date=end.strftime('%Y%m%d'))
        adjs = []
        for _, row in df.iterrows():
